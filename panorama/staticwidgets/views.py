...
class TextWidgetCreateView(CsrfExemptMixin, LoginRequiredMixin, JSONResponseMixin, CreateView):
    """ Create View for TextWidget objects. """
    template_name = 'panorama/widget_form.html'
    ...
    def get_form_kwargs(self, *args, **kwargs):
        form_kwargs = super(TextWidgetCreateView, self).get_form_kwargs(*args, **kwargs)
        form_kwargs['user'] = self.request.user
        return form_kwargs
    ...
    def form_valid(self, form):
        ...
        If the form is valid, redirect to the supplied URL.
        """
        super(TextWidgetCreateView, self).form_valid(form)
        return self.render_json_response({
            'location': reverse('dashboard'),
            'html': '<h2>Success! The page will be immediately reloaded...</h2>'
        })
